...
class AutoEncoder(nn.Module):
    def __init__(self, inp_size, hid_size):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(inp_size, round((inp_size - hid_size) / 4)),
            #nn.Dropout(),
            nn.Tanh(),
            nn.Linear(round((inp_size - hid_size) / 4), hid_size),
            #nn.Dropout(),
            nn.Sigmoid()
        )

        self.decoder = nn.Sequential(
            nn.Linear(hid_size, round((inp_size - hid_size) / 4)),
            #nn.Dropout(),
            nn.Tanh(),
            nn.Linear(round((inp_size - hid_size) / 4), inp_size),
            #nn.Dropout(),
            nn.Sigmoid()
        )
    ...
    def loss_function(self, recon_x, x):
        ...
        :return: loss
        """
        loss = torch.abs((recon_x - x) ** 2).sum()

        return loss / len(x) + self.lmbda * (self.decoder_param_sum() + self.encoder_param_sum())
...
def test_work():
    ...
    assert np.all([isinstance(submodules[layer.split('.')[0]][int(layer.split('.')[1])], nn.Linear)
                   for layer in layers_with_params]), 'All layers with parameters must be nn.Linear'
    print('Success!')
